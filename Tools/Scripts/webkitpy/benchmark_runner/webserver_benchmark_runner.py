...
class WebServerBenchmarkRunner(BenchmarkRunner):
    ...
    def __init__(self, plan_file, local_copy, count_override, timeout_override, build_dir, output_file, platform, browser, browser_path, subtests=None, scale_unit=True, show_iteration_values=False, device_id=None, diagnose_dir=None, generate_pgo_profiles=False, profile_output_dir=None, trace_type=None, profiling_interval=None, trace_args=None, browser_args=None, http_server_type='twisted', http_server_port=0):
        ...
        super().__init__(plan_file, local_copy, count_override, timeout_override, build_dir, output_file, platform, browser, browser_path, subtests=subtests, scale_unit=scale_unit, show_iteration_values=show_iteration_values, device_id=device_id, diagnose_dir=diagnose_dir, generate_pgo_profiles=generate_pgo_profiles, profile_output_dir=profile_output_dir, trace_type=trace_type, profiling_interval=profiling_interval, trace_args=trace_args, browser_args=browser_args, http_server_type=http_server_type, http_server_port=http_server_port)
    ...
    def _run_one_test(self, web_root, test_file, iteration):
        ...
        try:
            ...
            if '?' not in url:
                url = url.replace('&', '?', 1)
            if enable_profiling:
                context = self._browser_driver.profile(self._profile_output_dir, profile_filename, self._profiling_interval, trace_type=self._trace_type, trace_args=self._trace_args)
            else:
                context = NullContext()
            with context:
